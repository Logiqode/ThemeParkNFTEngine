.PHONY: up down migrate migrate-up migrate-down migrate-version healthy build test test-integration lint tidy clean reset
...
healthy:
	...
	@docker compose -f deployments/docker-compose.yml up -d
	@timeout $${TIMEOUT:-60} bash -c 'until [ $$(docker compose -f deployments/docker-compose.yml ps --format json | grep -c "\"Health\": \"healthy\"") -eq $$(docker compose -f deployments/docker-compose.yml ps --format json | grep -c "Service") ]; do sleep 2; done'
